...
#' ------------------------------------------------------------------------------
#'
# A script to simulate samples of a known populations for the purpose of comparison.
...
#simulating a population of potential
#plots for all stands in the project
popList[[1]] <- rtruncnorm(n = 100000,
                             sd = standSim$stdBAPA,
                             mean = standSim$BAPA,
                             a = 0)
...
cruise20 <- bind_rows(lapply(1:20, cruise_stand)) %>%
  mutate(CI = (sdtemp / sqrt(15)) * qt(1 - 0.1 / 2, df = 14))
#### Dataframe of cruise20 used in actual ForestrySource article:
# cruise20 <- {structure(list(sampleNum = 1:20, meantemp = c(174.369177627875,
...
#                                               170.606174766682, 175.75824174835, 165.281346873727), sdtemp = c(31.0971085556902,
...
#                                                                                                                33.7479392750561, 27.9132744620394, 35.4746547132257), CI = c(14.1419798631626,
...
#                                                                                                                                                                              15.3474937001417, 12.6940729762754, 16.1327491817655), Pass = c(TRUE,
#                                                                                                                                                                                                                                              TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, TRUE, TRUE, TRUE, TRUE,
#                                                                                                                                                                                                                                              FALSE, FALSE, FALSE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE), overlapPct = c(0.894736842105263,
...
#                                                                                                                                                                                                                                                                                                                       0.894736842105263, 0.789473684210526)), .Names = c("sampleNum",
#                                                                                                                                                                                                                                                                                                                                                                          "meantemp", "sdtemp", "CI", "Pass", "overlapPct"), row.names = c(NA,
...
#how many of the mean point estimates of the other cruises were within Cruise1's CI?
...
##So if you cruise a stand
## and then cruise the exact same stand again, with the same number of plots
## and same sampling design:
...
cruiseExample <- cruise20 %>%
  mutate(inCI = ifelse(meantemp < compHigh &
                         meantemp > compLow, 'Mean within original cruise CI', 'Mean outside original cruise CI')) %>%
           mutate(inCI = ifelse(sampleNum==2, 'Original cruise', inCI))
       
cruiseExample$inCI <- factor(cruiseExample$inCI, levels = c('Original cruise',
